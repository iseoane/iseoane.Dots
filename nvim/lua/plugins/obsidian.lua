return {
  ...
  enabled = function()
    ...
    -- or when the notes vault doesn't exist on this machine.
    return not vim.g.disable_obsidian and vim.fn.isdirectory(vim.fn.expand("~/.config/obsidian")) == 1
  end,
  dependencies = {
    -- Required.
    ...
    workspaces = {
      {
        ...
        path = vim.fn.expand("~/.config/obsidian"), -- Path to the notes directory ($HOME is unset in pwsh on Windows)
      },
    },
    completion = {
      cmp = true,
